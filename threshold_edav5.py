...
SCORE_HINTS = ["candidatescore", "matchprobability", "matchscore", "mlaasscore", "score"]

def find_score_array(df):
    """Find the ONE array whose element struct carries a score field.

    party_matches holds several unrelated arrays (dedup actions, rules,
    determinations, transitive_nodes, candidates). Exploding the wrong one
    yields no score; exploding several yields a CROSS PRODUCT and silently
    inflates the pair count. So we explode only the score-bearing array -
    in practice partySearch.candidates.
    """
    for f in df.schema.fields:
        if isinstance(f.dataType, ArrayType) and isinstance(f.dataType.elementType, StructType):
            for ch in f.dataType.elementType.fieldNames():
                if any(h == _norm0(ch) for h in SCORE_HINTS) or "score" in _norm0(ch):
                    return f.name
    return None

def explode_score_array(df, max_rounds=3):
    """Expand structs, then explode ONLY the score-bearing array. Repeats in
    case the score array is nested under another struct level."""
    df = expand_structs(df)
    for _ in range(max_rounds):
        target = MANUAL_EXPLODE_COL or find_score_array(df)
        if not target:
            break
        print("  exploding score-bearing array:", target)
        df = df.withColumn(target, F.explode_outer(F.col("`" + target + "`")))
        df = expand_structs(df)
        if find_score_array(df) is None:
            break
    return df
...
def _is_id(c):
    if c in ES_META:                      # _id is the ES doc id, NOT a party id
        return False
    n = _norm0(c)
    if "identityid" in n:                 # identity id is not a party id
        return False
    return ("partysourceid" in n or "partyid" in n or "candidateid" in n)

def _rank(c):
    """Prefer partySourceId over other id-ish columns."""
    n = _norm0(c)
    return (0 if "partysourceid" in n else 1, len(n))

id_cols  = [c for c in pairs_flat.columns if _is_id(c)]
cand_ids = sorted([c for c in id_cols if "candidate" in _norm0(c)], key=_rank)
decl_ids = sorted([c for c in id_cols if "candidate" not in _norm0(c)], key=_rank)

COLS["right_id"] = cand_ids[0] if cand_ids else resolve(pairs_flat.columns, "right_id")
COLS["left_id"]  = decl_ids[0] if decl_ids else resolve(
    [c for c in pairs_flat.columns if c != COLS["right_id"]], "left_id")
