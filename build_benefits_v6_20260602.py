...
__BUILD_VERSION__ = "2026-05-27-v6.12-map-missing-benefits"
...
PLAN_LEVEL_CATEGORIES = {
    ...
    # Plan-level deductibles, premiums, MOOP detail (benefit IDs 610-616)
    # Ground-truth shows these as separate rows with distinct sortOrders 500611..500616
    "Annual Plan Deductible":               610,
    "Annual Plan Deductible HMO":           610,
    "Annual Plan Deductible HMOPOS":        610,
    "Part D Drug Coverage Premium":         611,
    "Part D Premium":                       611,
    "Plan Premium":                         612,
    "Total Plan Premium":                   612,
    "Part C Premium":                       613,
    "Plan Part C Premium":                  613,
    "Medicare Part B Premium":              614,
    "Part B Premium":                       614,
    "Part B Buy-Down":                      615,
    "Part B Premium Reduction":             615,
    "Optional Supplemental Premium":        616,

    # Plan description / overview (benefit 620)
    "Plan Description":                     620,
    "Plan Characteristics":                 620,
    "Plan Type":                            620,

    # Cost-share summary (benefit 630)
    "Cost Sharing":                         630,
    "Cost-Sharing":                         630,
    "Standard Bid Detail":                  630,

    # Rx tier breakdowns (benefits 700, 711, 712)
    # 700 = tier names, 711/712 = retail/mail copays per tier
    "Rx Setup":                             710,
    ...
    "Rx Tiers":                             700,
    "Rx Tier Labels":                       700,
    "Rx Retail Cost Share":                 711,
    "Rx Retail":                            711,
    "Rx Mail Order Cost Share":             712,
    "Rx Mail Order":                        712,
    "Rx Mail":                              712,
    "Rx Preferred Mail Order":              755,
    "Rx Insulin":                           755,

    # Rx catastrophic / non-preferred (benefits 730, 740)
    "Rx Catastrophic Coverage":             730,
    "Rx Catastrophic":                      730,
    "Rx Non-Preferred":                     740,
    "Rx Tier 4":                            740,
    "Rx Non-Preferred Drug":                740,

    # Standard bid (already present)
    "Standard Bid":                         600,

    # Extras: foot care (1070), vaccines (1080)
    "Foot Care":                            1070,
    "Foot Care - Routine":                  1070,
    "Routine Foot Care":                    1070,
    "Podiatry Routine":                     1070,
    "Vaccines":                             1080,
    "Part D Vaccines":                      1080,
    "Adult Vaccines":                       1080,

    # Specific outpatient service (1301)
    "Mental Health":                        1301,
    "Outpatient Mental Health":             1301,

    # Vision allowance (1500)
    "Vision":                               1500,
    "Routine Vision":                       1500,
    "Eyewear":                              1500,
    "Eyeglasses":                           1500,
    ...
    # Star ratings (2110, 2111, 2112)
    "Star Rating":                          2110,
    "Overall Star Rating":                  2110,
    "Health Plan Star Rating":              2111,
    "Drug Plan Star Rating":                2112,
    "Part C Star Rating":                   2111,
    "Part D Star Rating":                   2112,
}
...
def _group_rows_by_benefit(plan_rows: pd.DataFrame,
    ...
    log_unmapped: if True, prints a summary of categories that didn't match any
                  benefit ID. These are rows the LLM never sees - if a benefit
                  is missing from output, check here first.
    """
    tagged_rows: list[tuple[int, bool, int]] = []  # (benefit_id, is_oon, row_idx)
    unmapped_categories: dict[str, int] = {}  # category -> count of dropped rows
    ...
    for idx, row in plan_rows.iterrows():
        ...
        # Otherwise this row isn't mapped - track it for diagnostics
        if cat:
    ...
    if log_unmapped and unmapped_categories:
        ...
        for cat, count in sorted(unmapped_categories.items(), key=lambda x: -x[1])[:15]:
            print(f"    {count:>4}  {cat[:90]}")
        if n_cats > 15:
            print(f"    ... and {n_cats - 15} more")
