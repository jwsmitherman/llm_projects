...
SCHEMA_SAMPLE_ROWS = 300           # A# is ~6% filled; need enough rows to see it

# --------------------------- PERFORMANCE -------------------------------------
# The single biggest cost is NOT data volume - it is that from_json over the
# _source blobs is LAZY, so every count/chart/groupBy re-parses every record
# from scratch. MATERIALIZE parses once, writes parquet to scratch, and reads
# that back. Everything downstream then hits columnar parquet instead of JSON.
#
# This writes FILES to a scratch path. It does NOT create catalog tables.
MATERIALIZE = True
SCRATCH_DIR = "dbfs:/tmp/irq_threshold_cache"   # any writable path; not Unity Catalog
REFRESH_CACHE = False        # True = re-parse from source even if scratch exists

# DEV_MODE: run on a slice first to prove the pipeline, then flip to False.
# Pairs are capped; parties are kept whole so the attribute joins still resolve.
DEV_MODE = True
DEV_PAIR_LIMIT = 250_000     # cap on scored pairs in dev mode

# Clustering is the other expensive step. Each threshold = a full CC pass.
CLUSTER_THRESHOLDS = [0.98, 0.90]   # add 0.95 / 0.92 back for the full sweep
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 1b. FLATTEN _source  (RECURSIVE, down to leaf fields)
#
# The real schema is nested more than one level, e.g. party_matches._source is
#   {"partySearch": {...declared party...},
#    "dedupResultsData": [ {candidateId, candidateScore}, ... ]}
# so a one-level expand yields only ['dedupResultsData','partySearch'] and every
# lookup fails. We therefore:
#   * parse _source (JSON string -> struct) using schema_of_json over a sampled
#     JSON ARRAY literal, so sparse keys (A# at 6% fill) survive the merge;
#   * recursively expand structs to leaves, naming them parent_child;
#   * explode arrays ON PAIRS ONLY (one row per scored pair). Arrays on PARTIES
#     are left alone - exploding them would duplicate parties and corrupt every
#     fill rate and count downstream.
# DataFrame API only: no sparkContext, no rdd.
# -----------------------------------------------------------------------------
...
def parse_source(df, sample_rows=SCHEMA_SAMPLE_ROWS):
    t = df.schema["_source"].dataType
    if isinstance(t, StructType):
        return df.select("_id", "_index", "_source.*")
    if isinstance(t, StringType):
        return (df.withColumn("_src", F.from_json(F.col("_source"), _infer_struct_ddl(df, sample_rows)))
                  .select("_id", "_index", "_src.*"))
    raise TypeError("unexpected _source type: " + str(t))

def expand_structs(df, max_depth=8):
    """Recursively flatten StructType columns to leaves: parent_child."""
        ...
        for f in df.schema.fields:
            if f.name in structs:
                for ch in f.dataType.fieldNames():
                    cols.append(F.col("`" + f.name + "`.`" + ch + "`").alias(f.name + "_" + ch))
            else:
...
# Exactness ranking. party_matches contains BOTH:
#   partySearch.candidates[]                      -> {partySourceId, candidateScore}
#   dedup_results.rules[].partiesWithScores[]     -> {partySourceId, score}
# The field name "partiesWithScores" merely CONTAINS "score", so a naive
# substring match chases the per-rule array and never reaches candidateScore.
# Rank 0 (exact candidateScore) beats rank 1 (exact score) beats rank 2 (fuzzy).
SCORE_EXACT_BEST = ["candidatescore", "matchprobability", "mlaasscore"]
SCORE_EXACT_OK   = ["score", "matchscore", "probability"]

def _norm0(s):
    return re.sub(r"[^a-z0-9]", "", s.lower())

def _score_rank_of_field(name):
    n = _norm0(name)
    if n in SCORE_EXACT_BEST:
        return 0
    if n in SCORE_EXACT_OK:
        return 1
    if "score" in n:
        return 2
    return None

def scalar_score_col(df):
    """A already-usable scalar score column, if one exists."""
    best, best_rank = None, 99
    for f in df.schema.fields:
        if isinstance(f.dataType, (ArrayType, StructType)):
            continue
        r = _score_rank_of_field(f.name.split("_")[-1])
        if r is not None and r < best_rank:
            best, best_rank = f.name, r
    return best if best_rank <= 1 else None      # require an exact-ish leaf

def find_score_array(df):
    """Pick the array whose element struct carries the BEST score field."""
    best, best_rank = None, 99
    for f in df.schema.fields:
        if not (isinstance(f.dataType, ArrayType) and isinstance(f.dataType.elementType, StructType)):
            continue
        for ch in f.dataType.elementType.fieldNames():
            r = _score_rank_of_field(ch)
            if r is not None and r < best_rank:
                best, best_rank = f.name, r
    return best

def explode_score_array(df, max_rounds=4):
    """Expand structs, then explode ONLY the best score-bearing array. Stops as
    soon as a scalar score column exists, so we never explode several unrelated
    arrays (which would produce a CROSS PRODUCT and inflate the pair count)."""
    df = expand_structs(df)
    for _ in range(max_rounds):
        if MANUAL_EXPLODE_COL:
            target = MANUAL_EXPLODE_COL if MANUAL_EXPLODE_COL in df.columns else None
        else:
            if scalar_score_col(df):
                break
            target = find_score_array(df)
        if not target:
            break
        print("  exploding score-bearing array:", target)
        df = df.withColumn(target, F.explode_outer(F.col("`" + target + "`")))
        df = expand_structs(df)
        if MANUAL_EXPLODE_COL:
            break
    return df

# PARTIES / IDENTITIES: expand structs only. Do NOT explode (would duplicate rows).
...
# PAIRS: expand AND explode -> one row per scored candidate pair.
print("flattening PAIRS:")
_pairs_parsed = expand_structs(parse_source(spark.table(PAIRS_TBL)))
print("  array columns present:",
      [f.name for f in _pairs_parsed.schema.fields if isinstance(f.dataType, ArrayType)])
pairs_flat = explode_score_array(_pairs_parsed)

for label, df in [("PARTIES", parties_flat), ("PAIRS", pairs_flat), ("IDENTITIES", ident_flat)]:
    print("\n=== " + label + " flattened (" + str(len(df.columns)) + " cols) ===")
    print(df.columns)

# --- materialize: parse the JSON ONCE, then read columnar parquet ------------
def materialize(df, name):
    """Write the flattened frame to scratch parquet and read it back.

    Without this, from_json re-parses every _source blob on every action -
    which is why the notebook crawls. Writes FILES, not catalog tables.
    """
    if not MATERIALIZE:
        return df
...
if DEV_MODE:
    print("DEV_MODE: capping pairs at", DEV_PAIR_LIMIT)
    pairs_flat = pairs_flat.limit(DEV_PAIR_LIMIT)

parties_flat = materialize(parties_flat, "parties")
pairs_flat   = materialize(pairs_flat,   "pairs" + ("_dev" if DEV_MODE else ""))
...
# -----------------------------------------------------------------------------
SYNONYMS = {
    "party_id": ["partysourceid", "partyid", "partyidentifier", "partykey", "id"],
    "score":    ["candidatescore", "matchprobability", "matchscore", "mlaasscore", "probability", "score"],
    "left_id":  ["partysourceid", "declaredpartyid", "instigatingpartyid", "leftid", "partyid"],
    "right_id": ["candidatepartysourceid", "candidateid", "candidatepartyid", "matchedpartyid", "rightid"],
    ...
    "status":   ["recordstatus", "status", "partystatus", "identitystatus"],
...
def _leaf(s):
    """Last path segment of a flattened name: partySearch_partyId -> partyid."""
    return _norm(s.split("_")[-1])

# Short/ambiguous keys: leaf-exact ONLY. Substring matching on these produces
# false positives - e.g. "eid" is a substring of "partySour[ceid]" and would
# silently bind eid to the party id column.
NO_SUBSTRING_KEYS = {"eid", "i94", "ssn", "fin", "cob", "coc", "dob", "form"}

# Elasticsearch envelope columns are never real fields.
ES_META = {"_id", "_index", "_score", "_type", "_source"}

def resolve(cols, key):
    """Leaf-exact first (names are parent_child), then full-name exact, then
    substring - except for short keys, where substring is disabled."""
    cols = [c for c in cols if c not in ES_META]
    cands = SYNONYMS.get(key, [])
    for cand in cands:
        for c in cols:
            if _leaf(c) == cand:
                return c
...
# The candidate (right) id lives on the exploded candidate struct; the declared
# (left) id lives on the search/declared struct. Resolve RIGHT first, then pick
# LEFT from whatever is left over, so they can never collapse onto one column.
COLS["score"] = resolve(pairs_flat.columns, "score")

# Both sides of the pair are called something like "partySourceId": the declared
# party is partySearch_partySourceId, the candidate is
# partySearch_candidates_partySourceId. Disambiguate on the "candidate" prefix,
# NOT on the field name, or left and right collapse onto the same column.
def _is_id(c):
    if c in ES_META:                       # _id is the ES doc id, not a party id
        return False
    n = _norm0(c)
    if "identityid" in n:
        return False
    return ("partysourceid" in n or "partyid" in n or "candidateid" in n)

id_cols = [c for c in pairs_flat.columns if _is_id(c)]

# The candidate (right) id lives on the SAME struct path as the score, e.g.
#   partySearch_candidates_candidateScore  ->  partySearch_candidates_partySourceId
# Keying off the shared prefix works no matter what the field is called; keying
# off the token "candidate" does not (the real column may not contain it).
score_prefix = COLS["score"].rsplit("_", 1)[0] + "_" if COLS.get("score") and "_" in COLS["score"] else ""

same_path  = [c for c in id_cols if score_prefix and c.startswith(score_prefix)]
other_path = [c for c in id_cols if c not in same_path]

def _rank_left(c):
    n = _norm0(c)
    # prefer the declared/instigating party on the search struct
    return (0 if "partysearch" in n else 1, len(n))

COLS["right_id"] = same_path[0] if same_path else (
    sorted([c for c in id_cols if "candidate" in _norm0(c)], key=len)[:1] or [None])[0]
COLS["left_id"] = (sorted(other_path, key=_rank_left)[:1] or [None])[0]

print("id columns on PAIRS:", id_cols)
print("  score:", COLS["score"])
print("  declared (left):", COLS["left_id"], "| candidate (right):", COLS["right_id"])
...
pairs_dx = pairs_dx.withColumn("n_strong_agree", strong_expr).cache()
print("decorated pairs:", pairs_dx.count())   # materialize once; sections 4/5/7/8 reuse it
...
# -----------------------------------------------------------------------------
MAX_EDGES_DRIVER = 20_000_000   # guard: refuse to pull more than this to the driver

def connected_components(edges):
    """edges: Spark DataFrame(src, dst) -> Spark DataFrame(id, component).

    Solved on the DRIVER with scipy. Rationale: an iterative label-propagation
    loop in Spark accumulates query-plan lineage across iterations and blows up
    (OOM on the plan string), and GraphFrames is a cluster-library dependency we
    can't assume. The edge list here is small enough to fit comfortably in driver
    memory - ~4.3M candidate pairs is only tens of MB as integer pairs - and
    scipy's connected_components is exact and fast.
    """
    ...
    if n_edges == 0:
        return None
    if n_edges > MAX_EDGES_DRIVER:
        raise MemoryError(
            f"{n_edges:,} edges exceeds MAX_EDGES_DRIVER ({MAX_EDGES_DRIVER:,}). "
            "Filter to a subset (e.g. declared-party pairs only) or raise the cap.")

    e = edges.select("src", "dst").toPandas()

    # map arbitrary ids -> contiguous integer codes
    codes, uniques = pd.factorize(pd.concat([e["src"], e["dst"]], ignore_index=True))
    half = len(e)
    si, di = codes[:half], codes[half:]
    n = len(uniques)

    g = coo_matrix((np.ones(half, dtype=np.int8), (si, di)), shape=(n, n))
    _, labels = scc(g, directed=False)

    out = pd.DataFrame({"id": uniques, "component": labels.astype(int)})
...
for t in CLUSTER_THRESHOLDS:
    ...
    if cc is None:
        continue
    sizes = cc.groupBy("component").count().cache()
