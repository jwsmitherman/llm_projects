...
LLM_SAMPLE_SEED = 42
target_sel = target.select(F.col(id_col).alias("_order_id"),
                           F.col(FREE_TEXT_COL).alias("_text"))
target_total = target_sel.count()
if LLM_SAMPLE_N and target_total > LLM_SAMPLE_N:
    target_sel = target_sel.orderBy(F.rand(LLM_SAMPLE_SEED)).limit(LLM_SAMPLE_N)
target_pdf = target_sel.toPandas()
...
if LLM_SAMPLE_N and target_total > LLM_SAMPLE_N:
    print(f"  RANDOM SAMPLE (seed {LLM_SAMPLE_SEED}) - {target_total - _n_calls:,} target orders "
          "carry llm_status='not_run'. Same seed returns the same sample on a rerun.")
    print("  Agreement rates from this sample generalise to the target set; raw counts do not.")
