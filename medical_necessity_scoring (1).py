...
# - A factual necessity label - necessary / not_necessary / no_documentation - against CMS
#   criteria, with an explicit **indeterminate** middle to be refined over time.
...
#   not_necessary    = text present with a clinical reason but only filler, no Group A concept.
#   no_documentation = the field is empty. Nothing was recorded to assess - not a finding.
...
df = df.withColumn(
    ...
    F.when(
        ...
        ~F.col("_has_text"),
        F.lit("no_documentation"),       # empty field - nothing to assess
    ).when(
        ~F.col("any_a") & F.col("any_filler"),
        F.lit("not_necessary"),          # text present, filler only - cite: [RSN] AM600 ; [MLN]
...
# modifier ("transport provided, payment not requested"). not_necessary = filler only, no reason
# (a GY candidate); no_documentation = empty field, nothing recorded; necessary = a documented
# reason is present; indeterminate = a partial or unread signal. This describes the order text at
# order time, not a billing determination.
df = df.withColumn(
    "gy_disposition",
    F.when(F.col("necessity_class") == "not_necessary", F.lit("no documented reason - GY candidate"))
     .when(F.col("necessity_class") == "no_documentation", F.lit("no documentation recorded"))
...
for cls in ["necessary", "indeterminate", "not_necessary", "no_documentation"]:
    print("=" * 70)
    print(cls.upper())
    display(
...
definitions_pdf = pd.DataFrame([
    ...
    ["necessity_class = no_documentation", "The free-text field is empty. Nothing was recorded to assess - not a finding about the trip.", "review-process 4 Aug"],
    ...
    ["gy_disposition", "GY process relation: not_necessary = no documented reason (GY candidate); no_documentation = nothing recorded; necessary = documented reason present; indeterminate = partial, review.", "review-process 4 Aug"],
...
if BUILD_REVIEW_WORKBOOK:
    ...
    guide_pdf = pd.DataFrame([
        ...
        ["necessity_class", "Label for the order text: necessary / indeterminate / not_necessary / no_documentation."],
    ...
    review_tabs = {
        ...
        "No documentation": review_frame(df.filter(F.col("necessity_class") == "no_documentation")),
