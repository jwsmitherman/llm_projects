...
for i, cty in enumerate(FOCUS_COUNTIES, 1):
    volume, funnel, time_summary, monthly, cond, crit, bullets = slide_data(cty)
    packs[cty] = (volume, funnel, time_summary, monthly, cond, crit, bullets)
    narratives.append({"slide": i, "county": cty, "bullet_1": bullets[0], "bullet_2": bullets[1]})
    print(f"\n==================  SLIDE {i}: {cty.upper()} COUNTY  ({TIMEFRAME})  [{GATE_TXT}]  ==================")
...
try:
    with pd.ExcelWriter(f"{OUTPUT_DIR}/two_slide_data.xlsx", engine="openpyxl") as xw:
        ...
        for cty in FOCUS_COUNTIES:
            volume, funnel, time_summary, monthly, cond, crit, _ = packs[cty]
