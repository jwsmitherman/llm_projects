...
def _infer_struct_ddl(df, sample_rows=500):
    """Infer the JSON schema of _source WITHOUT spark.sparkContext / df.rdd,
    both of which are blocked on Unity Catalog shared-access clusters.

    Trick: join the sampled records into a single JSON ARRAY literal. Spark's
    schema_of_json merges fields across array elements, so we get the union of
    keys, then we strip the outer ARRAY<> to recover the STRUCT<...> DDL.
    """
    ...
    ddl = (spark.range(1)
                .select(F.schema_of_json(F.lit(arr)).alias("s"))
                .collect()[0]["s"])                      # e.g. ARRAY<STRUCT<a:STRING,...>>

    up = ddl.upper()
    if up.startswith("ARRAY<") and ddl.endswith(">"):
        return ddl[len("ARRAY<"):-1]                     # -> STRUCT<...>
    return ddl                                           # already a struct

def flatten_source(df, sample_rows=500):
    ...
    if isinstance(src_type, StringType):
        struct_ddl = _infer_struct_ddl(df, sample_rows)
        return (df.withColumn("src", F.from_json(F.col("_source"), struct_ddl))
...
# party_matches very likely stores ONE ROW PER DECLARED PARTY with an ARRAY of
# scored candidates nested inside (consistent with 433 declared records ->
# 17,827 score submissions). Explode it so we get one row per scored PAIR.
from pyspark.sql.types import ArrayType

array_cols = [f.name for f in pairs_flat.schema.fields if isinstance(f.dataType, ArrayType)]
print("array columns on PAIRS (explode candidates):", array_cols)

# >>> TODO: set to the array field holding scored candidates (e.g. "matches"),
# or None if party_matches is already one row per pair.
EXPLODE_COL = array_cols[0] if len(array_cols) == 1 else None

if EXPLODE_COL:
    print("exploding PAIRS on '" + EXPLODE_COL + "' -> one row per scored pair")
    pairs_flat = (pairs_flat
                  .withColumn("_m", F.explode(F.col(EXPLODE_COL)))
                  .select("*", "_m.*")
                  .drop("_m", EXPLODE_COL))
...
# NOTE on inference: the schema is inferred from a SAMPLE of _source rows. A#
# is only ~6% populated, so a small sample could miss it entirely and silently
# drop the column. sample_rows defaults to 500 for that reason. If a field you
# expect is absent from the printout above, raise sample_rows and re-run.
...
# -----------------------------------------------------------------------------
# Checkpoint dir via conf, NOT spark.sparkContext.setCheckpointDir (blocked on
# Unity Catalog shared-access clusters, same restriction that broke §1b).
spark.conf.set("spark.checkpoint.dir", "/tmp/cpms_cc_checkpoint")

# GraphFrames must be installed on the cluster (Libraries -> Maven:
# graphframes:graphframes:0.8.3-spark3.5-s_2.12, or the matching build).
# If it isn't available, skip §8 rather than pip-installing at runtime.
