...
__BUILD_VERSION__ = "2026-05-27-v6.10-comissio-endpoint"


# ----------------------------------------------------------------------------
# Configuration - all overridable via env vars (recommended: use a .env file)
# ----------------------------------------------------------------------------
# Defaults point at the comissio-poc deployment which has 4M TPM (vs ~20K on
# joshu-mebu0lvp-swedencentral). With this much headroom, throttling is not a
# concern and we can run with high concurrency.
#
# To override, set in .env or process env:
#   AZURE_OPENAI_ENDPOINT
#   AZURE_OPENAI_API_KEY
#   AZURE_OPENAI_DEPLOYMENT
#   AZURE_OPENAI_API_VERSION
AZURE_OPENAI_ENDPOINT    = os.getenv("AZURE_OPENAI_ENDPOINT", "https://comissio-poc.openai.azure.com/")
AZURE_OPENAI_API_KEY     = os.getenv("AZURE_OPENAI_API_KEY", "")  # NEVER hardcode - put in .env
AZURE_OPENAI_DEPLOYMENT  = os.getenv("AZURE_OPENAI_DEPLOYMENT", "gpt-5-mini-ng")
AZURE_OPENAI_API_VERSION = os.getenv("AZURE_OPENAI_API_VERSION", "2024-12-01-preview")
AZURE_OPENAI_TEMPERATURE: Optional[float] = None  # gpt-5 family rejects explicit temp

# gpt-5-mini consumes completion tokens for internal reasoning before producing
# output text. 1500 was tight on the old endpoint; bump it to leave room for
# both reasoning and output. Empty responses with finish_reason=length are the
# signal that this is too low.
LLM_MAX_TOKENS    = int(os.getenv("LLM_MAX_TOKENS", "4000"))

# Retry settings - kept conservative even though TPM is high, since transient
# 5xx and network errors still happen.
MAX_RETRIES       = int(os.getenv("MAX_RETRIES", "5"))
BASE_BACKOFF_S    = float(os.getenv("BASE_BACKOFF_S", "2.0"))
MAX_BACKOFF_S     = float(os.getenv("MAX_BACKOFF_S", "30.0"))

# Concurrency - the comissio deployment supports 4M TPM, so we can push much
# higher than the old joshu endpoint allowed. Estimated per-call cost ~3-5K
# tokens; 16 concurrent calls = ~80K tokens in flight, well under budget.
# Reduce these via env vars if a different endpoint is used.
MAX_CONCURRENCY   = int(os.getenv("MAX_CONCURRENCY", "16"))    # Was 2 on the throttled endpoint
CONCURRENT_PLANS  = int(os.getenv("CONCURRENT_PLANS", "4"))    # Was 1 on the throttled endpoint
