...
# ---- SPEED CONTROL ----------------------------------------------------------
# QUICK_MODE: pull a FIXED small number of rows via .limit() on the raw read, so
# Spark stops after N rows instead of scanning the whole table. Runs in ~1-2 min
# and gives real data points. Turn OFF (False) for the representative sample run.
QUICK_MODE      = True
QUICK_PAIRS     = 5_000       # match records to read (each explodes to a few pairs)
QUICK_PARTIES   = 50_000      # parties to read for the attribute join + fill stats

DEV_MODE        = True        # used only when QUICK_MODE is False
PAIR_SAMPLE_FRAC   = 0.005    # 0.5% of match records. Lower = faster.
PARTY_SAMPLE_FRAC  = 0.05     # 5% of parties for fill-rate/DOB stats
DEV_SEED           = 42
PAIR_HARD_CAP      = 10_000   # absolute ceiling on pairs after sampling (safety net)
...
if QUICK_MODE:
    # Fixed tiny reads. .limit() on the raw scan lets Spark stop early -> ~1-2 min.
    pairs_raw   = pairs_raw.limit(QUICK_PAIRS)
    parties_raw = parties_raw.limit(QUICK_PARTIES)
    ident_raw   = ident_raw.limit(QUICK_PARTIES)
    parties_smp = parties_raw
    print(f"QUICK_MODE: pairs<= {QUICK_PAIRS:,}, parties<= {QUICK_PARTIES:,}")
elif DEV_MODE:
    # sample small, then hard-cap. The .limit lets Spark stop scanning once it
    # has enough rows, so it never reads the whole match table.
...
tag = "quick" if QUICK_MODE else ("dev" if DEV_MODE else "full")
pairs_flat        = materialize(pairs_flat, f"pairs_{tag}")
parties_flat      = materialize(parties_flat, f"parties_{tag}")
ident_flat        = materialize(ident_flat, f"identities_{tag}")
# In QUICK_MODE the parties read is the SAME limited slice used for the join, so
# some sampled pairs may reference parties outside the slice -> their attributes
# come back null. That is expected in quick mode; the point is speed + a few real
# data points, not full coverage. Use DEV_MODE (sample) or a full run for coverage.
