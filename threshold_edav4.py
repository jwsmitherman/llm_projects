...
def explode_arrays(df, max_explodes=2):
    """Explode array columns (then re-expand the resulting structs)."""
    for _ in range(max_explodes):
        arrays = [f.name for f in df.schema.fields if isinstance(f.dataType, ArrayType)]
        if not arrays:
            break
        a = MANUAL_EXPLODE_COL if (MANUAL_EXPLODE_COL in arrays) else arrays[0]
        print("  exploding array column:", a)
        df = df.withColumn(a, F.explode_outer(F.col("`" + a + "`")))
...
pairs_flat = explode_arrays(expand_structs(parse_source(spark.table(PAIRS_TBL))))
...
def resolve(cols, key):
    """Leaf-exact match first (the flattened names are parent_child), then
    full-name exact, then substring."""
...
COLS["right_id"] = resolve(pairs_flat.columns, "right_id")
left_pool = [c for c in pairs_flat.columns if c != COLS["right_id"]]
COLS["left_id"]  = resolve(left_pool, "left_id")
